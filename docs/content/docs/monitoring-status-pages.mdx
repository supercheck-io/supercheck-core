...
<Callout type="tip">
  All monitor types support configurable check frequencies, alert thresholds,
...
<Callout type="warning">
  Smart alert limiting prevents notification spam by limiting failure alerts to
  3 per incident and consolidating recovery notifications.
</Callout>
...
<Callout type="tip">
  Status pages use UUID-based subdomains to ensure unique access and prevent
  conflicts.
</Callout>
...
<Callout type="warning">
  SSE connections require proper authentication and authorization to ensure
