...
import { NotificationService, NotificationPayload, NotificationProvider } from "src/notification/notification.service";
...
export class MonitorAlertService {
    ...
    async sendNotification(monitorId: string, type: 'recovery' | 'failure', reason: string, metadata: any = {}) {
        this.logger.log(`[NOTIFY] Manual notification trigger for monitor ${monitorId}, type: ${type}`);
        ...
        if (!providers || providers.length === 0) {
            this.logger.log(`[NOTIFY] No notification providers configured for monitor ${monitorId}`);
            return;
        }
        ...
        switch (type) {
            case 'recovery':
                ...
                if (!monitor.alertConfig.alertOnRecovery) {
                    this.logger.log(`[NOTIFY] Recovery notifications disabled for monitor ${monitorId}`);
                    return;
                }
                break;
            case 'failure':
                ...
                if (!monitor.alertConfig.alertOnFailure) {
                    this.logger.log(`[NOTIFY] Failure notifications disabled for monitor ${monitorId}`);
                    return;
                }
                break;
            default:
        ...
        this.logger.log(`[NOTIFY] Sending ${notificationType} notifications to ${providers.length} providers`);

        const { success: successCount, failed: failedCount } = await this.notificationService.sendNotificationToMultipleProviders(providers as NotificationProvider[], notificationPayload);

        this.logger.log(`[NOTIFY] Notification results: ${successCount} success, ${failedCount} failed`);
