...
const DATABASE_URL = process.env.DATABASE_URL || `postgresql://${DB_USER}:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${DB_NAME}`;
    ...
    try {
        ...
        if (err.message.includes('does not exist')) {
            ...
            try {
                ...
                logError(`Failed to create database '${DB_NAME}': ${createErr.message}`);
                return false;
            }
        } else {
...
async function runMigrations() {
    ...
    try {
        ...
        for (const migrationFile of migrationFiles) {
            ...
            const migrationHash = require('crypto').createHash('md5').update(migrationContent).digest('hex');
