...
export const statusPages = pgTable("status_pages", {
  ...
  status: varchar("status", { length: 50 }).$type<StatusPageStatus>().notNull().default("draft"),
  ...
  // Branding
  cssBodyBackgroundColor: varchar("css_body_background_color", { length: 7 }).default("#ffffff"),
  cssFontColor: varchar("css_font_color", { length: 7 }).default("#333333"),
  cssLightFontColor: varchar("css_light_font_color", { length: 7 }).default("#666666"),
 ...
 */
export const statusPageComponentGroups = pgTable("status_page_component_groups", {
  id: uuid("id").primaryKey().$defaultFn(() => sql`uuidv7()`),
...
export const statusPageComponents = pgTable("status_page_components", {
  id: uuid("id").primaryKey().$defaultFn(() => sql`uuidv7()`),
  statusPageId: uuid("status_page_id").notNull().references(() => statusPages.id, { onDelete: "cascade" }),
  componentGroupId: uuid("component_group_id").references(() => statusPageComponentGroups.id, { onDelete: "set null" }),
  monitorId: uuid("monitor_id").references(() => monitors.id, { onDelete: "set null" }),
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  status: varchar("status", { length: 50 }).$type<ComponentStatus>().notNull().default("operational"),
...
export const incidents = pgTable("incidents", {
  ...
  createdByUserId: uuid("created_by_user_id").references(() => user.id, { onDelete: "no action" }),
  name: varchar("name", { length: 255 }).notNull(),
  status: varchar("status", { length: 50 }).$type<IncidentStatus>().notNull().default("investigating"),
  impact: varchar("impact", { length: 50 }).$type<IncidentImpact>().notNull().default("minor"),
  ...
  autoTransitionToMaintenanceState: boolean("auto_transition_to_maintenance_state").default(true),
  autoTransitionToOperationalState: boolean("auto_transition_to_operational_state").default(true),
  ...
  autoTransitionDeliverNotificationsAtStart: boolean("auto_transition_deliver_notifications_at_start").default(true),
  autoTransitionDeliverNotificationsAtEnd: boolean("auto_transition_deliver_notifications_at_end").default(true),
  reminderIntervals: varchar("reminder_intervals", { length: 100 }).default("[3,6,12,24]"),
...
export const incidentComponents = pgTable("incident_components", {
  id: uuid("id").primaryKey().$defaultFn(() => sql`uuidv7()`),
  incidentId: uuid("incident_id").notNull().references(() => incidents.id, { onDelete: "cascade" }),
  componentId: uuid("component_id").notNull().references(() => statusPageComponents.id, { onDelete: "cascade" }),
...
export const incidentTemplates = pgTable("incident_templates", {
  ...
  title: varchar("title", { length: 255 }).notNull(),
  body: text("body").notNull(),
  componentGroupId: uuid("component_group_id").references(() => statusPageComponentGroups.id),
  updateStatus: varchar("update_status", { length: 50 }).default("investigating"),
 ...
 */
export const incidentTemplateComponents = pgTable("incident_template_components", {
  id: uuid("id").primaryKey().$defaultFn(() => sql`uuidv7()`),
  templateId: uuid("template_id").notNull().references(() => incidentTemplates.id, { onDelete: "cascade" }),
  componentId: uuid("component_id").notNull().references(() => statusPageComponents.id, { onDelete: "cascade" }),
  createdAt: timestamp("created_at").defaultNow(),
});
...
export const statusPageSubscribers = pgTable("status_page_subscribers", {
  ...
  skipConfirmationNotification: boolean("skip_confirmation_notification").default(false),
 ...
 */
export const statusPageComponentSubscriptions = pgTable("status_page_component_subscriptions", {
  id: uuid("id").primaryKey().$defaultFn(() => sql`uuidv7()`),
  subscriberId: uuid("subscriber_id").notNull().references(() => statusPageSubscribers.id, { onDelete: "cascade" }),
  componentId: uuid("component_id").notNull().references(() => statusPageComponents.id, { onDelete: "cascade" }),
  createdAt: timestamp("created_at").defaultNow(),
});
 ...
 */
export const statusPageIncidentSubscriptions = pgTable("status_page_incident_subscriptions", {
  id: uuid("id").primaryKey().$defaultFn(() => sql`uuidv7()`),
  incidentId: uuid("incident_id").notNull().references(() => incidents.id, { onDelete: "cascade" }),
  subscriberId: uuid("subscriber_id").notNull().references(() => statusPageSubscribers.id, { onDelete: "cascade" }),
  createdAt: timestamp("created_at").defaultNow(),
 ...
 */
export const statusPageMetrics = pgTable("status_page_metrics", {
  id: uuid("id").primaryKey().$defaultFn(() => sql`uuidv7()`),
  statusPageId: uuid("status_page_id").notNull().references(() => statusPages.id, { onDelete: "cascade" }),
  componentId: uuid("component_id").references(() => statusPageComponents.id, { onDelete: "cascade" }),
  ...
  dateIdx: uniqueIndex("status_page_metrics_date_component_idx").on(table.statusPageId, table.componentId, table.date),
}));
...
export const statusPageComponentsRelations = relations(statusPageComponents, ({ one, many }) => ({
  statusPage: one(statusPages, {
    fields: [statusPageComponents.statusPageId],
    references: [statusPages.id],
  }),
  monitor: one(monitors, {
    fields: [statusPageComponents.monitorId],
...
export const statusPageComponentGroupsInsertSchema = createInsertSchema(statusPageComponentGroups);
export const statusPageComponentGroupsSelectSchema = createSelectSchema(statusPageComponentGroups);

export const statusPageComponentsInsertSchema = createInsertSchema(statusPageComponents);
export const statusPageComponentsSelectSchema = createSelectSchema(statusPageComponents);
...
export const incidentTemplatesInsertSchema = createInsertSchema(incidentTemplates);
export const incidentTemplatesSelectSchema = createSelectSchema(incidentTemplates);

export const statusPageSubscribersInsertSchema = createInsertSchema(statusPageSubscribers);
export const statusPageSubscribersSelectSchema = createSelectSchema(statusPageSubscribers);

export const statusPageMetricsInsertSchema = createInsertSchema(statusPageMetrics);
export const statusPageMetricsSelectSchema = createSelectSchema(statusPageMetrics);
