...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "[AI Fix Debug] Markdown report URL:",
      markdownReportUrl,
      "for testId:",
      testId
    );
    ...
    if (markdownReportUrl) {
      ...
      console.log(
        ...
        "errors in markdown:",
        errorClassifications.map((e) => ({
          message: e.message,
          category: e.classification?.category,
          aiFixable: e.classification?.aiFixable,
        }))
      );
    } else {
      // Step 4b: Fallback to HTML report parsing
      console.log(
        "[AI Fix Debug] No markdown report found, trying HTML report for testId:",
        testId
      );
      ...
      if (!htmlContent) {
        console.log(
          "[AI Fix Debug] No HTML report found either for testId:",
          testId
        );
        return NextResponse.json(
          {
            ...
            message:
              "No test report (markdown or HTML) found. Please ensure the test has failed and reports are generated.",
      ...
      console.log(
        "[AI Fix Debug] HTML content preview (first 500 chars):",
      ...
      console.log("[AI Fix Debug] Parsed HTML report:", {
        ...
        errors: parsedHtmlReport.errors.map((e) => ({
          message: e.message,
          details: e.details.substring(0, 100),
        })),
      });
    ...
    if (!fixDecision.shouldAttemptFix) {
      ...
      if (errorClassifications.length === 0 && failedScript) {
        console.log(
          "[AI Fix Debug] No markdown errors found, attempting basic AI analysis with script"
        );
        try {
          ...
          console.log(
            "[AI Fix Debug] Basic analysis also failed:",
            basicAnalysisError
          );
        }
      }
    ...
    return NextResponse.json({
      ...
      errorAnalysis: {
        totalErrors: errorClassifications.length,
        fixableErrors: errorClassifications.filter(
          (ec) => ec.classification?.aiFixable
        ).length,
        topIssues: errorClassifications
          .map((ec) => ec.classification?.category)
          .filter(Boolean)
          .slice(0, 3),
      },
...
async function getMarkdownReportUrl(testId: string): Promise<string | null> {
  try {
    ...
    console.log(
      "[AI Fix Debug] Looking for .md files in data folder using AWS SDK, prefix:",
      dataFolderPrefix
    );
    ...
    try {
      ...
      console.log(
        "[AI Fix Debug] AWS SDK listing response - KeyCount:",
        response.KeyCount,
        "IsTruncated:",
      ...
      if (response.Contents && response.Contents.length > 0) {
        ...
        const allFiles = response.Contents.map((obj) => obj.Key).filter(
          Boolean
        );
        ...
        console.log(
          "[AI Fix Debug] No .md files found in",
          response.Contents.length,
          "objects"
        );
      } else {
        console.log(
          "[AI Fix Debug] No objects found in data directory with prefix:",
          dataFolderPrefix
        );
      }
    } catch (awsError) {
      console.log(
        "[AI Fix Debug] AWS SDK listing failed:",
        awsError instanceof Error ? awsError.message : "Unknown error"
      );
      console.log("[AI Fix Debug] AWS SDK error details:", awsError);
    }

    console.log(
      "[AI Fix Debug] No .md files found in data directory for testId:",
      testId
    );
    return null;
  } catch (error) {
