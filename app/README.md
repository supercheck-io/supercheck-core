...
```bash
# Run Redis with Docker (Example)
docker run -d --name redis-supertest -p 6379:6379 redis
```

#### 4. Start MinIO (Optional, for S3 features)
...
# Test Execution Concurrency (BullMQ workers)
...
3. **Direct Playwright CLI:** Run tests directly using the standard Playwright command line. This bypasses the web UI, API, and the BullMQ queueing system.
...
Tests initiated via the Playground or Jobs UI utilize a BullMQ queue for reliable execution and management.
